...
본 서비스는 **학생 / 선생 / 관리자**의 3개 역할을 중심으로 동작하며,  
...
| **관리자 (Admin)** | 기관/학급 계정·권한 관리, 성과 대시보드 모니터링, 정책 운영 | AI 사용량·학습 성과 지표 기반 품질 관리, 운영 피드백 초안 보조 | 관리 화면에서 지표를 점검하고 정책을 조정해 다음 수업 운영과 AI 활용 기준에 반영 |

### AI 작동 흐름 (학생·선생·관리자 중심)

1. **역할 분기**: 학생, 선생, 관리자로 진입 경로를 분리  
2. **데이터 수집**: 학습 활동·수업 운영·관리 정책 데이터를 통합 저장  
3. **AI 처리**: 추천·생성·분석·위험예측 기능을 AI 엔진에서 수행  
4. **결과 전달**: Streamlit 내부 **역할 기반 세션 관리**(`st.session_state`의 `AUTH_UID`, `AUTH_ROLE` 등)로 접근 권한을 확인한 뒤 역할별 화면을 제공  
5. **개선 루프**: 관리자가 성과 지표를 기반으로 운영 정책과 활용 가이드를 고도화

### 공모전 제출용 다이어그램 (통합본 1개)

- Miro 통합본:  
  https://miro.com/app/board/uXjVJpVOrvw=/?moveToWidget=3458764667409085455

### GitHub README에서 바로 보기 (Mermaid)

> GitHub는 Miro iframe 임베드를 직접 렌더링하지 않으므로, README에서는 아래 Mermaid로 즉시 미리보기를 제공합니다.

```mermaid
flowchart LR
  A(["서비스 시작"]) --> B{"역할 분기: 학생 / 선생 / 관리자"}

  B -->|학생| S1["학습 목표·진도 입력 및 과제 수행"]
  S1 --> D[("학습·수업·성과 데이터 저장")]
  D --> AI_ENGINE["AI 분석·생성 엔진 (추천·생성·예측)"]
  AI_ENGINE --> S2["개인화 추천 제공"]
  S2 --> S3["AI 튜터 피드백 반영 및 재학습"]
  S3 --> D

  B -->|선생| T1["수업 목표·단원 계획 등록"]
  T1 --> AI_ENGINE
  AI_ENGINE --> T2["수업안·문항·루브릭 초안 제공"]
  T2 --> T3["학습분석 리포트 기반 맞춤 지도"]
  T3 --> D

  B -->|관리자| M1["계정·권한·운영 정책 관리"]
  M1 --> M2["운영 지표 기반 AI 품질 관리"]
  M2 --> AI_ENGINE

  AI_ENGINE --> AUTH["역할 기반 세션 관리 (st.session_state)"]
  AUTH --> Z(["역할별 맞춤 결과 제공"])
  Z --> M2
```
