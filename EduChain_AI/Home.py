...
**학원·교육 기관(기업) 단위**로 운영·수업·학습을 한 흐름에서 다루는 **교육용 웹 애플리케이션**입니다.
운영자는 기관 설정과 구성원·콘텐츠를 관리하고, 교사는 수업을 설계·운영하며, 학생은 배정된 수업을 수강합니다.
**AI**는 교안·퀴즈 생성, 학습 질의 응답 등에 활용되며, 기관별 **AI 토큰 사용량**을 추적할 수 있습니다.

### 역할별로 볼 수 있는 것

| 역할 | 주요 화면 |
|------|-----------|
| **운영자** | **관리** — 기업 등록·선택, 기본 정보·플랜·교사·학생·콘텐츠·**AI 토큰 활용량** |
| **교사** | **Teacher** — 수업 선택 후 개요·학생 관리·수업 통계·수업 관리·AI 토큰 |
| **학생** | **Student** — 개요·수업 개요·수업 수강·통합 퀴즈 |

로그인하면 **역할에 맞는 메뉴만** 왼쪽 상단에 보입니다. 다른 역할 화면을 보려면 해당 계정으로 로그아웃 후 다시 로그인해야 합니다.

### 시스템 구성

- **클라이언트:** Streamlit(브라우저)
- **계정·데이터:** Firebase Authentication · Cloud Firestore
- **AI:** Google Gemini 등(기관·수업 맥락에 맞춘 호출)

---
"""
    )
    st.info(
        "왼쪽 사이드바 **「로그인 · 회원가입」** 또는 상단 **Login**에서 시작하세요. "
        "회원가입 후 **운영자**는 **관리**에서 기업을 만들고, **교사·학생**은 초대 코드로 소속을 신청할 수 있습니다."
    )
